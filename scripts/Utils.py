...
def defColumns(line):
  ...
  # initialize
  Cols = ['PT', 'AU', 'TI', 'SO', 'DT', 'DE', 'ID', 'C1', 'CR', 'TC', 'J9', 'PD', 'PY', 'VL', 'IS', 'BP', 'WC', 'UT', 'AB'];
  defCols = dict();
		...
		try:
			...
			for line in fd.readlines():
				...
				if (line != ""):
					...
					if(len(s)>12): aline.de_keywords = s[12]
					if(len(s)>13): aline.id_keywords = s[13]
					if(len(s)>14): aline.issue = s[14]
					if(len(s)>15): aline.abstract = s[15]
